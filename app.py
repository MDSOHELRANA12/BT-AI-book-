...
# --- ৪. সিএসএস ডিজাইন (অরিজিনাল কাঠামো) ---
st.markdown("""
    <style>
        ...
        for index, v in enumerate(data):
            ...
            with c2:
                if st.button(f"➕ Follow", key=f"f_{v_id}"):
                    supabase.table("followers_count").update({"followers": v.get("followers", 0) + 1}).eq("id", v_id).execute()
                    st.rerun()
            ...
            # বড় ব্যানার অ্যাড (প্রতি ২ ভিডিও পর)
            if (index + 1) % 2 == 0:
                st.markdown(f'''
