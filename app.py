...
# --- ১. ডাটাবেস কানেকশন (সোহেল ভাই, আপনার সব আইডি এখানে সেট আছে) ---
...
# --- ৩. বিজ্ঞাপনের আইডি (আপনার ৪টি ব্যানার কোড) ---
...
# --- ৪. চকচকে ডিজাইন (কালো ব্যাকগ্রাউন্ডে সাদা লেখা) ---
st.markdown("""
    <style>
...
# --- ৬. গ্লোবাল ফিড (ভিডিওর সাথে লাইক, ভিউ এবং বিজ্ঞাপন) ---
    ...
    try:
        ...
        if v_data.data:
            for index, v in enumerate(v_data.data):
                ...
                # আপনার ডাইরেক্ট লিঙ্ক বাটন
                d_link = "https://www.profitablecpmratenetwork.com/tgt6azn6?key=e753cbd6d9bae06d67051ed846419521"
                st.markdown(f'<a href="{d_link}" target="_blank" class="direct-link-btn">📽️ Watch & Earn Money</a>', unsafe_allow_html=True)
                ...
                c3.write(f"📊 Earn: ${v.get('views', 0) * 0.01:.2f}")
                st.markdown('</div>', unsafe_allow_html=True)

                # বিজ্ঞাপনগুলো অটোমেটিক আসবে
        ...
        st.error("ভিডিও লোড হতে সমস্যা হচ্ছে।")

# --- ৭. ভিডিও পাবলিশ (সোহেল ভাই, এটা এখন ১০০% কাজ করবে) ---
    ...
    if st.button("Publish Now") and v_file:
        with st.spinner("Processing..."):
            ...
            try:
                ...
                st.success("Success! Video Published, Sohel Bhai!")
                st.balloons()
            except Exception as e:
                st.error(f"Error: {e}")

# --- ৮. ওয়ালেট (রিয়েল ডলারের হিসাব) ---
...
# --- ৯. প্রোফাইল ও অনার কন্ট্রোল ---
elif choice == "👤 Profile & Security":
    st.title("👤 Profile Details")
    st.markdown('<div class="card"><h3>Name: MD SOHEL RANA</h3><p>Status: Verified Admin</p></div>', unsafe_allow_html=True)

elif choice == "⚙️ Owner Control":
    st.title("⚙️ Owner Access")
    pwd = st.text_input("Admin Password", type="password")
    if pwd == "S$s123456789112233":
        st.success("Welcome back, Owner Sohel Rana!")
import
