...
# ৩. সোশ্যাল বার (Social Bar) - এটি স্ক্রিনে নড়াচড়া করবে
components.html("""
...
if tab == "🌍 World Feed":
    try:
        ...
        for index, v in enumerate(data):
            ...
            # প্রতিটি ভিডিওর নিচে আপনার সেই লাল বাটন
            ...
            # --- এখানে আপনার সব পুরনো ব্যানার অ্যাডগুলো সিরিয়াল অনুযায়ী বসানো হলো ---
            ...
            with c2:
                if st.button(f"➕ Follow", key=f"f_{v['id']}"):
                    supabase.table("followers").insert({"user": st.session_state.user, "following": v['uploader_name']}).execute()
                    st.rerun()

            st.markdown('</div>', unsafe_allow_html=True)

            # ৬. প্রতি ৩টি ভিডিও পর পর সেই নড়াচড়া করা বিশেষ রিওয়ার্ড অ্যাড
            if (index + 1) % 3 == 0:
                components.html("""
                    <div style="text-align:center; padding:10px; background:#111; border-radius:10px;">
